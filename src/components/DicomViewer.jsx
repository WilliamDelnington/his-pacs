...
export default function DicomViewer({ studyId }) {
  ...
  const fetchInstances = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await orthancClient.get(`/studies/${studyId}/instances`);
  ...
  if (loading) {
    return (
      <Placeholder icon="loader" title="Loading instances…" />
    );
  }
  ...
  if (instances.length === 0) {
    return (
      <Placeholder
        ...
        sub={`Study ${studyId?.slice(0, 18)}… returned no DICOM instances.`}
      />
    );
  }
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-gray-700 bg-gray-950 flex flex-col">
      ...
      {/* Thumbnail strip (only when >1 instance) */}
      {instances.length > 1 && (
